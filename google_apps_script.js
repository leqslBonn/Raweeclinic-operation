...
function doPost(e) {
  ...
  try {
    ...
    switch (body.action) {
      ...
      case 'addExpense': return addExpense_(data);
      ...
      case 'approveExpense': return approveExpense_(data);
      case 'setAppointmentStatus': return setAppointmentStatus_(data);
      case 'voidTransaction': return voidTransaction_(data);
      case 'setEntityActive': return setEntityActive_(data);
      case 'repairPhoneFormatting': return repairPhoneFormatting_();
      case 'setModuleEnabled': return setModuleEnabled_(data);
...
function initializeOperationalWorkbook_() {
  const schemas = {
    ...
    Appointments: ['appointment_id','appointment_date','start_time','end_time','customer_id','service_id','provider_id','room','status','confirmation_status','note','created_at','created_by','updated_at','updated_by'],
    Visits: ['visit_id','visit_date','customer_id','appointment_id','service_id','provider_id','treatment_area','product_brand','product_lot','quantity','inventory_item_id','stock_quantity','clinical_note','photo_folder_url','consent_recorded','next_followup_date','created_at','created_by'],
    Transactions: ['transaction_id','transaction_date','customer_id','appointment_id','visit_id','service_id','package_id','course_id','transaction_type','description','quantity','unit_price','discount','net_amount','paid_amount','balance_due','payment_method','payment_status','staff_id','note','created_at','void_reason','voided_at','voided_by'],
...
function getSystemData_() {
  const names = ['Customers','Employees','Appointments','FollowUps','Transactions','Attendance','Services','Visits','Packages','CustomerCourses','CourseUsage','Expenses','Inventory','StockMovements','SOP','Settings'];
  const keys = ['customers','employees','appointments','followups','transactions','attendance','services','visits','packages','customer_courses','course_usage','expenses','inventory','stock_movements','sop','settings'];
...
function addVisit_(data) {
  ...
  data.visit_id = data.visit_id || makeId_('VIS');
  if (data.inventory_item_id && Number(data.stock_quantity || 0) > 0) {
    const stockResult = applyStockMovement_({ movement_date: data.visit_date, item_id: data.inventory_item_id, movement_type: 'USE', quantity: data.stock_quantity, reference_type: 'Visit', reference_id: data.visit_id, staff_id: data.staff_id || '', note: 'ตัด Stock จากการรับบริการ', created_at: new Date() });
    if (!stockResult.ok) return json_(stockResult);
  }
...
function addExpense_(data) {
  ...
  if (!data.expense_date || !String(data.category || '').trim() || !String(data.description || '').trim() || amount <= 0) return json_({ ok: false, error: 'Date, category, description and positive amount are required' });
  data.amount = amount;
  data.status = 'Pending';
  return appendRecord_('Expenses', data, 'EXP');
}

function approveExpense_(data) {
  const status = String(data.status || '');
  if (!data.expense_id || ['Approved','Rejected'].indexOf(status) < 0) return json_({ ok: false, error: 'Expense and approval status are required' });
  return updateRecord_('Expenses', 'expense_id', {
    expense_id: data.expense_id,
    status: status,
    approved_by: data.performed_by || 'owner',
    note: data.note || '',
    updated_at: new Date()
  });
}

function setAppointmentStatus_(data) {
  const status = String(data.status || '');
  if (!data.appointment_id || ['Scheduled','Confirmed','Completed','Cancelled'].indexOf(status) < 0) return json_({ ok: false, error: 'Appointment and valid status are required' });
  return updateRecord_('Appointments', 'appointment_id', {
    appointment_id: data.appointment_id,
    status: status,
    note: data.note || '',
    updated_at: new Date(),
    updated_by: data.performed_by || 'owner'
  });
}

function voidTransaction_(data) {
  if (!data.transaction_id) return json_({ ok: false, error: 'Transaction is required' });
  const transaction = records_('Transactions').find(row => String(row.transaction_id) === String(data.transaction_id));
  if (!transaction) return json_({ ok: false, error: 'Transaction not found' });
  if (String(transaction.payment_status) === 'Void') return json_({ ok: false, error: 'Transaction is already void' });
  return updateRecord_('Transactions', 'transaction_id', {
    transaction_id: data.transaction_id,
    payment_status: 'Void',
    void_reason: String(data.reason || '').trim(),
    voided_at: new Date(),
    voided_by: data.performed_by || 'owner'
  });
}

function setEntityActive_(data) {
  const entities = {
    service: ['Services','service_id'], package: ['Packages','package_id'],
    employee: ['Employees','employee_id'], inventory: ['Inventory','item_id'], sop: ['SOP','sop_id']
  };
  const config = entities[String(data.entity || '')];
  if (!config || !data.id) return json_({ ok: false, error: 'Entity and id are required' });
  const update = {}; update[config[1]] = data.id; update.active = Boolean(data.active); update.updated_at = new Date(); update.updated_by = data.performed_by || 'owner';
  if (data.entity === 'employee') update.work_status = data.active ? 'Active' : 'Inactive';
  return updateRecord_(config[0], config[1], update);
}

function repairPhoneFormatting_() {
  const targets = [['Customers','phone'],['Employees','phone']]; let repaired = 0;
  targets.forEach(item => {
    const sheet = sheet_(item[0]); const headers = sheet.getRange(1,1,1,sheet.getLastColumn()).getValues()[0].map(String); const column = headers.indexOf(item[1]) + 1;
    if (!column || sheet.getLastRow() < 2) return;
    const range = sheet.getRange(2,column,sheet.getLastRow()-1,1); const values = range.getDisplayValues().map(row => {
      let phone = cleanPhone_(row[0]); if (phone.length === 9) { phone = '0' + phone; repaired++; } return [phone];
    });
    range.setNumberFormat('@'); range.setValues(values);
  });
  return json_({ ok: true, repaired: repaired });
}

function setModuleEnabled_(data) {
  const allowed = ['ลูกค้า','ติดตามลูกค้า','นัดหมาย','เข้ารับบริการ','บริการและโปรโมชั่น','การเงิน','คอร์ส','ค่าใช้จ่าย','พนักงาน','SOP & Checklist','Stock'];
  const moduleName = String(data.module || '');
  if (allowed.indexOf(moduleName) < 0) return json_({ ok: false, error: 'Unknown module' });
  return upsertRecord_('Settings', 'setting_id', { setting_id: 'module:' + moduleName, setting_value: data.enabled ? 'TRUE' : 'FALSE', updated_at: new Date(), updated_by: data.performed_by || 'owner' }, 'SET');
}

function recordCustomerActivity_(data) {
  const allowedStatuses = ['ลูกค้าใหม่','ด่วน','เกินกำหนด','รอติดตาม','รอยืนยัน','นัดแล้ว','สำเร็จ','งดติดต่อ'];
  if (!data.customer_id || allowedStatuses.indexOf(String(data.status)) < 0) return json_({ ok: false, error: 'Customer and valid status are required' });
  if (!records_('Customers').some(row => String(row.customer_id) === String(data.customer_id))) return json_({ ok: false, error: 'Customer not found' });
  const now = new Date();
  ...
  appendRecord_('FollowUps', { customer_id: data.customer_id, due_date: data.next_followup_date || '', followup_type: data.followup_type || 'ติดตามลูกค้า', priority: data.priority || '', assigned_to: data.actor_id || data.performed_by || '', status: data.status, contact_result: data.contact_result || '', note: data.note || '', completed_at: data.status === 'สำเร็จ' ? now : '', completed_by: data.actor_id || data.performed_by || '', created_at: now }, 'FUP', true);
...
function applyStockMovement_(data) {
  if (!data.item_id || !data.movement_type || Number(data.quantity) <= 0) return { ok: false, error: 'Item, type and positive quantity are required' };
  ...
  if (onHand < 0) return { ok: false, error: 'Insufficient stock' };
  ...
  return { ok: true, movement_id: data.movement_id, on_hand: onHand };
}

function addStockMovement_(data) { return json_(applyStockMovement_(data)); }
...
function appendByHeaders_(sheet, data) { const headers=sheet.getRange(1,1,1,sheet.getLastColumn()).getValues()[0].map(String),row=headers.map(key=>Object.prototype.hasOwnProperty.call(data,key)?data[key]:'');const target=sheet.getRange(sheet.getLastRow()+1,1,1,headers.length);headers.forEach((key,index)=>{if(/phone|line_id/i.test(key))target.getCell(1,index+1).setNumberFormat('@');});target.setValues([row]); }
